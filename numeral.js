	...
	function formatTime (n, format) {
		...
		if (format.indexOf('y') > -1) {
			var years	= Math.floor(n._value/oneyear),
				...
				majformat	= format.split('y'),
				yearsformated	= formatNumber(years, majformat[0]);
			return yearsformated + 'y' + months + 'm' + weeks + 'w' + days + 'd' + ((hours < 10) ? '0' + hours : hours) + ':' + ((minutes < 10) ? '0' + minutes : minutes) + ':' + ((seconds < 10) ? '0' + seconds : seconds);
			...
			var months	= Math.floor(n._value/onemonth),
				...
				majformat	= format.split('m'),
				monthsformated	= formatNumber(months, majformat[0]);
			return monthsformated + 'm' + weeks + 'w' + days + 'd' + ((hours < 10) ? '0' + hours : hours) + ':' + ((minutes < 10) ? '0' + minutes : minutes) + ':' + ((seconds < 10) ? '0' + seconds : seconds);
			...
			var weeks	= Math.floor(n._value/oneweek),
				...
				majformat	= format.split('w'),
				weeksformated	= formatNumber(weeks, majformat[0]);
			return weeksformated + 'w' + days + 'd' + ((hours < 10) ? '0' + hours : hours) + ':' + ((minutes < 10) ? '0' + minutes : minutes) + ':' + ((seconds < 10) ? '0' + seconds : seconds);
			...
			var days	= Math.floor(n._value/oneday),
				...
				majformat	= format.split('d'),
				daysformated	= formatNumber(days, majformat[0]);
			return daysformated + 'd' + ((hours < 10) ? '0' + hours : hours) + ':' + ((minutes < 10) ? '0' + minutes : minutes) + ':' + ((seconds < 10) ? '0' + seconds : seconds);
		} else {
			var hours = Math.floor(n._value/60/60),
				minutes = Math.floor((n._value - (hours * 60 * 60))/60),
				seconds = Math.round(n._value - (hours * 60 * 60) - (minutes * 60)),
				majformat	= format.split(':'),
				hoursformated	= formatNumber(hours, majformat[0]);
			return hoursformated + ':' + ((minutes < 10) ? '0' + minutes : minutes) + ':' + ((seconds < 10) ? '0' + seconds : seconds);
		}
	}

	function unformatTime (string) {
		var	string = string.toLowerCase();
			timeArray = string.split(/[ymwd:]/),
			timeArray = timeArray.reverse(),
			timeUnits = [ 1, 60, 3600, 86400, 604800, 2592000, 946080000 ],
			seconds = 0;
		if (timeArray.length <= 6) {
			for (i = 0; i < timeArray.length; i++) {
				if ( i == (timeArray.length - 1)) {
					timeArray[i] = unformatNumeral(timeArray[i]);
				}
				seconds = seconds + (timeArray[i] * timeUnits [i]);
			}
		}
